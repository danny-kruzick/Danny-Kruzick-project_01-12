...
# ╔═╡ 5a4f1548-cc0c-426f-9a0d-87c0c803b0ac
	...
	begin
		...
		# Rotation rates (rad/s) for required comparison
		Ω_slow = 2.0
		Ω_fast = 20.0
end
...
# Solved by derivation (see markdown above):
#   θ̈ = (Ω^2/L) (w1 + L sinθ) cosθ - (g/L) sinθ
function rotating_pendulum(du, u, p, t)
	...
	du[1] = theta_dot
	du[2] = (Ω^2 / L) * (w1 + L*sin(theta)) * cos(theta) - (g / L) * sin(theta)
end
...
begin
	# Package parameters into tuples to avoid global-variable issues in Pluto
	p_slow = (g, L, w1, Ω_slow)
	p_fast = (g, L, w1, Ω_fast)

	prob_slow = ODEProblem(rotating_pendulum, u0, time, p_slow)
	prob_fast = ODEProblem(rotating_pendulum, u0, time, p_fast)

	sol_slow = solve(prob_slow, Tsit5(); saveat=0.001)
	sol_fast = solve(prob_fast, Tsit5(); saveat=0.001)
end
...
# -------------------------
# Coordinates of the bob in the inertial frame
# (x,y rotate with the frame at prescribed Ω; θ(t) comes from the ODE solve)
# -------------------------
begin
...
# -------------------------
# Trajectory (x-z projection) for slow vs fast rotation
# -------------------------
begin
	plt = plot(x_slow, z_slow,
		xlabel="x (m)", ylabel="z (m)",
		title="Bob Trajectory (x–z projection)",
		linewidth=2, aspect_ratio=:equal, label="slow Ω=$(Ω_slow) rad/s")

	plot!(plt, x_fast, z_fast, linewidth=2, label="fast Ω=$(Ω_fast) rad/s")
	plt
end

# ╔═╡ 28d8019f-9eac-4220-a2b2-9a3ed69369df
# -------------------------
# Plot angle vs time (slow vs fast)
# -------------------------
begin
	plt = plot(sol_slow.t, sol_slow[1,:],
		xlabel="Time (s)", ylabel="θ (rad)",
		title="Angle vs Time (slow vs fast Ω)",
		label="slow Ω=$(Ω_slow) rad/s")

	plot!(plt, sol_fast.t, sol_fast[1,:], label="fast Ω=$(Ω_fast) rad/s")
	plt
end

# ╔═╡ 34f155bb-ddab-437f-8650-d3a9e2a740db
# -------------------------
# Plot angular velocity θ̇ vs time (slow vs fast)
# -------------------------
begin
	plt = plot(sol_slow.t, sol_slow[2,:],
		xlabel="Time (s)", ylabel="θ̇ (rad/s)",
		title="Angular Velocity vs Time (slow vs fast Ω)",
		label="slow Ω=$(Ω_slow) rad/s")

	plot!(plt, sol_fast.t, sol_fast[2,:], label="fast Ω=$(Ω_fast) rad/s")
	plt
end

# ╔═╡ 692e844c-86e6-4c5d-8b21-8e2f1dda94ce
# -------------------------
# Energies (optional): T(t), V(t), and Lagrangian ℒ(t) for slow vs fast
#
# Using the same cylindrical kinematics as the derivation:
#   T = 1/2 m [ L^2 θ̇^2 + (r Ω)^2 ],  r = w1 + L sinθ
#   V = m g (h1 - L cosθ)
# -------------------------
begin
	...
	plt = plot(sol_slow.t, T_s, xlabel="Time (s)", ylabel="Energy (J)",
		title="Energies vs Time", label="T (slow)", linewidth=2)
	...
	function make_animation(sol, Ω, x, y, z; w1=w1, L=L, h1=h1, stride=5)
		@animate for i in 1:stride:length(sol.t)
			plot(xlim=(-w1-L, w1+L),
				...
				zlim=(-h1, L+h1),
				legend=false,
				title="Spinning Pendulum (Ω=$(Ω) rad/s)")
			...
			plot!([0, w1*cos(Ω*sol.t[i])],
				  [0, w1*sin(Ω*sol.t[i])],
				  [0, 0],
				  linewidth=3)

			# Pendulum rod (pivot to bob)
			plot!([w1*cos(Ω*sol.t[i]), x[i]],
				  [w1*sin(Ω*sol.t[i]), y[i]],
				  [0, z[i]],
				  linewidth=3)
...
# ╔═╡ afa56048-da44-4cff-957d-e5ddf04780e1
begin
	gif(anim_slow, "rotating_pendulum_slow.gif", fps=30)
end

# ╔═╡ 1627fde2-028d-4f31-b906-1791e600a4e2
begin
	gif(anim_fast, "rotating_pendulum_fast.gif", fps=30)
end
