...
begin
	...
	sol_slow = solve(prob_slow, saveat=0.001)
	sol_fast = solve(prob_fast, saveat=0.001)
end
...
begin
	# Compare trajectory in x–z projection (matches the team's earlier trajectory view)
	plt = plot(x_slow, z_slow, xlabel="x (m)", ylabel="z (m)",
		title="Trajectory (x–z) — slow vs fast Ω", linewidth=2, aspect_ratio=:equal,
		label="slow Ω=$(Ω_slow)")
	plot!(plt, x_fast, z_fast, linewidth=2, label="fast Ω=$(Ω_fast)")
	plt
end
...
begin
	# Animations for slow and fast (two separate gif cells below so Pluto shows both)
	function make_animation(sol, Ω, x, y, z; w1=w1, L=L, h1=h1, stride=5)
		@animate for i in 1:stride:length(sol.t)
			plot(xlim=(-w1-L, w1+L), ylim=(-w1-L, w1+L), zlim=(-h1, L+h1),
				legend=false, title="Spinning Pendulum (Ω=$(Ω) rad/s)")

			plot!(x[1:i], y[1:i], z[1:i], linewidth=2)
			plot!([0,0], [0,0], [-h1,0], linewidth=2)

			plot!([0, w1*cos(Ω*sol.t[i])], [0, w1*sin(Ω*sol.t[i])], [0, 0], linewidth=3)
			plot!([w1*cos(Ω*sol.t[i]), x[i]], [w1*sin(Ω*sol.t[i]), y[i]], [0, z[i]], linewidth=3)
